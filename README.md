...
### Безопасность
- Авторизация по паролю (bcrypt, cost=12)
- 2FA (TOTP) + backup-коды
- Защита от брутфорса с экспоненциальной блокировкой
- Восстановление пароля по одноразовому токену (1 час, rate-limit)
- Логирование действий и проверок входа
- Подтверждение опасных действий в админ-командах
...
| `/2fa_verify <код>` | Подтверждение и включение 2FA |
| `/2fa_disable <код>` | Отключение 2FA |
| `/2fa <код/backup>` | Подтверждение 2FA при логине |
...
Бот использует `.env` и запускается как `python bot/main.py`.
